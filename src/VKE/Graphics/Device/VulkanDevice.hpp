...
struct VKE_HIDDEN VulkanDevice final {
    ...
    public:
        ...
        u32 getMemoryType(u32 type_bits, VkMemoryPropertyFlags properties, u32 *memory_type_index = VKE_NULL_PTR) const;
        u32 getQueueFamilyIndex(VkQueueFlagBits queue_flags) const;
        VkResult createLogicalDevice(VkPhysicalDeviceFeatures enabled_features, std::vector<const char *> enabled_extensions, void *next_chain,
            bool use_swap_chain = true, VkQueueFlags requested_queue_types = VK_QUEUE_GRAPHICS_BIT | VK_QUEUE_COMPUTE_BIT);
        VkResult createBuffer(VkBufferUsageFlags usage_flags, VkMemoryPropertyFlags memory_property_flags, VkDeviceSize size, VkBuffer *buffer, VkDeviceMemory *memory,
            void *data = VKE_NULL_PTR);
        VkResult createBuffer(VkBufferUsageFlags usage_flags, VkMemoryPropertyFlags memory_property_flags, Buffer *buffer, VkDeviceSize size, void *data = VKE_NULL_PTR);
        void copyBuffer(Buffer *src, Buffer *dst, VkQueue queue, VkBufferCopy *copy_region = VKE_NULL_PTR);
